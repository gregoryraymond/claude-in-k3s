...
- no use of unwrap() or expect() except in tests, use results and push errors up the stack
- limit uses of clone() except where absolutely necessary
- prefer functional approaches over raw loops
...
- use tests/feature for feature tests
- use tests/e2e for end to end tests
- use tests/fuzz for fuzz testing
- You should try and get 5 negative cases for each positive case, and when writing tests you should bias towards tests that fail due to breaking the code. Once the test is written and failing you should then fix the bug.
